...
HANDLE gf_thrd_open( char *name, unsigned char priority, gf_policy_t policy, int stack_size, 
					threadhandler_t handler, threadhandler_t Errorhandler, uint32_t wparam, uint32_t lparam )
{
	int i, rc;
...
#endif
			{
				printf( "Release self-exit thread<%s:%x>\n", m_all[i]->name, *(uint32_t *)&m_all[i] );			
	...
	if( info == NULL )
	{
		printf( "error: gf_thrd_open() failed\n" );
		if( m_mutex )
		{
			gf_mutex_unlock( m_mutex );
		}
	...
	if( rc != 0 )
	{
		...
		printf( "gf_thrd_open [%s]: set stack size failed! %d, using default size %ld\n", name, stack_size, defsize );
	}
...
{   
	...
	sthrdinfo_t *info = (sthrdinfo_t *)thrd;
	printf( "On Live:Thread<%s:%x> is exit!!!\n", info->name, (void *)info );
	pthread_exit(0);  
}  
#endif
/** 
