...
REOPEN: 
	GFCOM_LOG_DEBUG( "Init com device [%s] with rate [%d] timeout [%d]", dev, comClient->rate, comClient->timeout/1000);
	...
	{
		GFCOM_LOG_DEBUG( "Can't Open Serial Port!");
		gf_thrd_delay(1000);
		goto REOPEN;
	}
	//设置串口参数
  	if (set_Parity(comClient->socket,8,1,'N')== -1)
  	{
		...
		comClient->socket = -1;
    	GFCOM_LOG_DEBUG( "Set Parity Error");
		gf_thrd_delay(1000);
		goto REOPEN;
  	}
  	...
  	while(1)
  	{
        ...
        else if(activeCount > 0 && FD_ISSET(comClient->socket, &rfds))
        {
            if((nread = read(comClient->socket,comClient->recv_buf,sizeof(comClient->recv_buf))) <= 0)
            {
				close(comClient->socket);
				comClient->socket = -1;
				GFCOM_LOG_DEBUG( "Read data from serial port %s error reopen it", dev);
				gf_thrd_delay(500);
				goto REOPEN;
            }
        }
		...
		if(comClient->debug)
		{
			int i = 0;
			for(i = 0; i < nread; i++)
				GFCOM_LOG_DEBUG( "%02x ", comClient->recv_buf[i]);
			GFCOM_LOG_DEBUG( " length = %d\n\n",nread);
		}
...
bool gf_com_send_data(ComClient_t *comClient, unsigned char *data, int size)
{
	...
	if(rtn <= 0)
		GFCOM_LOG_DEBUG( "com write error\n");
