...
void udp_handle(int usd, char *data, int data_len, struct sockaddr *sa_source, int sa_len)
{
    ...
    if (debug) {
        ...
        GFNTPCLIENT_LOG_DEBUG("packet of length %d received\n",data_len);
        if (sa_source->sa_family==AF_INET) {
            GFNTPCLIENT_LOG_DEBUG("Source: INET Port %d host %s\n",
                ntohs(sa_in->sin_port),inet_ntoa(sa_in->sin_addr));
        } else {
            GFNTPCLIENT_LOG_DEBUG("Source: Address family %d\n",sa_source->sa_family);
        }
    }
#ifndef GOEFUL_TEST
...
{   
 time_t transmit_time;   
 struct tm *localtm;  

 memcpy(&transmit_time, data + 32, 4);   
 transmit_time = ntohl(transmit_time) - JAN_1970;   
 GFNTPCLIENT_LOG_DEBUG("transmit time : %lx\n", transmit_time);   
 //time(&time_tmp);   
 localtm= gmtime(&transmit_time);   
 GFNTPCLIENT_LOG_DEBUG("Year : %d month : %d dat : %d h : %d m : %d s:%d\n",   
...
#undef Data
...
#endif
        if (debug) {
            GFNTPCLIENT_LOG_DEBUG("set time to %lu.%.6lu\n", tv_set.tv_sec, tv_set.tv_usec);
        }
    }

    if (debug) {
    GFNTPCLIENT_LOG_DEBUG("LI=%d  VN=%d  Mode=%d  Stratum=%d  Poll=%d  Precision=%d\n",
        li, vn, mode, stratum, poll, prec);
    GFNTPCLIENT_LOG_DEBUG("Delay=%.1f  Dispersion=%.1f  Refid=%u.%u.%u.%u\n",
        sec2u(delay),sec2u(disp),
        refid>>24&0xff, refid>>16&0xff, refid>>8&0xff, refid&0xff);
    GFNTPCLIENT_LOG_DEBUG("Reference %u.%.10u\n", reftime.coarse, reftime.fine);
    GFNTPCLIENT_LOG_DEBUG("Originate %u.%.10u\n", orgtime.coarse, orgtime.fine);
    GFNTPCLIENT_LOG_DEBUG("Receive   %u.%.10u\n", rectime.coarse, rectime.fine);
    GFNTPCLIENT_LOG_DEBUG("Transmit  %u.%.10u\n", xmttime.coarse, xmttime.fine);
    GFNTPCLIENT_LOG_DEBUG("Our recv  %u.%.10u\n", arrival->coarse, arrival->fine);
    }
    ...
    if (debug) {
    GFNTPCLIENT_LOG_DEBUG("Total elapsed: %9.2f\n"
           "Server stall:  %9.2f\n"
           "Slop:          %9.2f\n",
        etime, stime, etime-stime);
    GFNTPCLIENT_LOG_DEBUG("Skew:          %9.2f\n"
           "Frequency:     %9d\n"
    ...
    GFNTPCLIENT_LOG_DEBUG("%d %5d.%.3d  %8.1f %8.1f  %8.1f %8.1f %9d\n",
...
void stuff_net_addr(struct in_addr *p, char *hostname)
{
	...
	GFNTPCLIENT_LOG_DEBUG("hostent-h_name:%s\n",ntpserver->h_name);
    if (ntpserver == NULL) {
        herror(hostname);
        exit(1);
    }
...
void primary_loop(int usd, int num_probes, int interval)
{
    ...
    if (debug) GFNTPCLIENT_LOG_DEBUG("Listening...\n");
    ...
    for (;;) {
        ...
        if (pack_len<0) {
            ...
            GFNTPCLIENT_LOG_DEBUG("Ooops.  pack_len=%d\n",pack_len);
            fflush(stdout);
        }
...
void do_replay(void)
{
    ...
    while (fgets(line,sizeof(line),stdin)) {
        ...
        if (n==7) {
            ...
            errorbar=etime+disp;
            if (debug) GFNTPCLIENT_LOG_DEBUG("contemplate %u %.1f %.1f %d\n",
            ...
            if (debug) GFNTPCLIENT_LOG_DEBUG("fake %f %d \n", fake_delta_time, simulated_freq);
