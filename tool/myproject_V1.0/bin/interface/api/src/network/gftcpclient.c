...
/*数据流接收线程*/
static bool tcpclient_onrecv( unsigned long wparam, unsigned long lparam )
{
	...
	if( pClient==NULL )
	{
		GFTCPCLIENT_LOG_DEBUG( "[%s:%d]Invalid Tcp client!\n",__FUNCTION__,__LINE__);
		return false;
	}
again:
	if(gf_tcp_select(pClient->socket, &readfds, NULL, NULL, 1000))
	{
		...
		if(pClient->check_entire)
		{
			if(strstr(pClient->buf, "<?xml"))
			{
			   ...
			   {
					// no finish recv
					GFTCPCLIENT_LOG_DEBUG("no recv all datas...,recv again\n");
					goto again;
			   }
			}
			else
		...
		if(rcv_size <= 0)
		{
			GFTCPCLIENT_LOG_DEBUG("[%s:%d]net recive failed\n", __FUNCTION__, __LINE__);
			pClient->datasize = 0;
			pClient->hThrd = NULL;
			if( pClient->pErrorHandler )
			{
				if( !pClient->pErrorHandler(pClient->socket, pClient->ip, pClient->port, pClient->lParam) )
				{
					closesocket(pClient->socket);
					pClient->socket = INVALID_SOCKET;
					if(pClient->databuf)
 ...
 * NOTES: 
 *************************************************************************/
bool gf_tcpclient_init( STcpClient* pClient,unsigned long ip,unsigned short port,PPacketHandler pPacketHandler,PErrorHandler pErrorHandler,unsigned long lParam )
{
	...
	pClient->pErrorHandler = pErrorHandler;
	...
	for( i=0; i<3;  i++ )
	{
		...
		/* 设为非阻塞模式 */
		if(gf_tcp_ioctl( skt, FIONBIO, (unsigned long*)&unblock ) < 0)
		{
			closesocket(skt);
            skt = -1;
			continue;
		...
		{
			closesocket(skt);
			GFTCPCLIENT_LOG_ERROR("[%s]connect retcode:%d errorcode=%d,skt = %d\n", __FUNCTION__,retcode, errno,skt);
            skt = -1;
			continue;
		}
		n=0;
WAIT:
		...
		{
			closesocket(skt);
            GFTCPCLIENT_LOG_ERROR("[%s]tcp select error retcode:%d,skt = %d\n",__FUNCTION__,retcode,skt);
            skt = -1;
			continue ;
		}
		...
		if(retcode == 0)
		{
			GFTCPCLIENT_LOG_ERROR("[%s:%d]select socket timeout!\n", __FUNCTION__, __LINE__);
			closesocket(skt);
            skt = -1;
			continue;
		}
		...
		else if( FD_ISSET( skt, &wset ) )
		{
			...
			if(getsockopt(skt, SOL_SOCKET, SO_ERROR, &err, (socklen_t *)&len) < 0)
				...
				{
					closesocket( skt );
					GFTCPCLIENT_LOG_ERROR("[%s:%d]getsocketopt failed!skt:%d\n",__FUNCTION__, __LINE__,skt);
                    skt = -1;
					continue;
				}
			...
			{
				closesocket( skt );
				GFTCPCLIENT_LOG_ERROR("[%s:%d] connect failed erron:%d,skt:%d!\n", __FUNCTION__, __LINE__, err,skt);
                skt = -1;
				continue;
			}
			break;
	...
	if(skt<0)
	{
		GFTCPCLIENT_LOG_DEBUG("[%s:%d]cannot connect server!\n", __FUNCTION__, __LINE__ );
	...
	pClient->hThrd = gf_thrd_open( "TcpClientRecv", 50, 0, 256*1024,/*(PThreadHandler)*/tcpclient_onrecv, NULL, (unsigned long)pClient, lParam);
	if( pClient->hThrd == NULL )
	{
		GFTCPCLIENT_LOG_DEBUG( "[%s:%d]cannot create task !\n", __FUNCTION__, __LINE__ );
		gf_tcpclient_exit(pClient);
		return false;
	}
	gf_thrd_resume( pClient->hThrd );
	GFTCPCLIENT_LOG_DEBUG("[%s:%d]connect server(%x:%x) suc.\n", __FUNCTION__, __LINE__, ip, port);
	return true;
}
...
int gf_tcpclient_send(STcpClient* pClient, char* buf, int size, int timeout)
{
    ...
    GFTCPCLIENT_LOG_INFO("tcpclient = 0x%x,socket = %d\n",pClient,pClient->socket);	
...
{
	return pClient->socket != -1 || pClient->hThrd != NULL;
}
