...
static size_t getuploadlengthfunc(void *ptr, size_t size, size_t nmemb, void *stream)   
{  
    int r;  
    long len = 0;  
...
static size_t getdownloadlengthfunc(void *ptr, size_t size, size_t nmemb, void *stream)   
{  
    int r;  
    long len = 0;  
//	GFFTPCLIENT_LOG_DEBUG( "head:%s", (char*)ptr);
...
static size_t writetomemfunc(void *ptr, size_t size, size_t nmemb, void *stream)  
{  
	if(m_downmem == NULL)
	{
	...
	if(m_downsize <= mem_filesize)
	{
		...
		GFFTPCLIENT_LOG_DEBUG( "size:%ld-%ld\t%s:%p:%s\n", size*nmemb, mem_filesize, __FUNCTION__, m_downmem, (char *)m_downmem);
		m_downmem += size*nmemb;
	}
	return size * nmemb;
}  

static size_t readmemfunc(void *ptr, size_t size, size_t nmemb, void *stream)  
{

	GFFTPCLIENT_LOG_DEBUG( "test:%s:%s\n", __FUNCTION__, (char *)stream);
	ptr = (char *)stream;
...
static int ftp_get_remotesize(CURL *curlhandle)
{
	int r;
    long filesize =0 ;  
    //设置头处理函数  
...
static int ftp_set_headopt(CURL *curlhandle, const char * remotepath, const char * username, const char * password, long timeout)  
{
...
static int ftp_upload_from_file(CURL *curlhandle, const char * localpath, long tries)  
{  
    FILE *f;  
    long uploaded_len = 0;  
...
static int ftp_upload_from_mem(CURL *curlhandle, const char *membuf, unsigned long size, long tries)  
{  
    long uploaded_len = 0;  
    CURLcode r = CURLE_GOT_NOTHING;  
    int c;  
...
// 下载  
static char *ftp_download_to_mem(CURL *curlhandle, long *download_size, long tries)  
{  
	mem_filesize = 0;
...
// 下载  
static int ftp_download_to_file(CURL *curlhandle, const char * localpath, long tries)  
{  
    FILE *f;  
    curl_off_t local_file_len = -1 ;  
    CURLcode r = CURLE_GOT_NOTHING;  
...
static int ftp_set_printfmodule(CURL *curlhandle, int out)
{
	//关闭显示详细信息
...
char *gf_ftp_download_to_mem(const char *ftpurl, long *msize, const char *username, const char *password, const int timeout)
{
	...
	if(msize > 0 && mem != NULL)
	{
		GFFTPCLIENT_LOG_DEBUG( "download suc\turl[%s]\tfilesize:%ld\nmem:%s\n", ftp_url, *msize, mem);
		m_downloadbuf = (char *)malloc(*msize);
		memset(m_downloadbuf, 0, *msize);
		memcpy(m_downloadbuf, mem, *msize);
	}
	else
		GFFTPCLIENT_LOG_DEBUG( "download err\turl[%s]\tfilesize:%ld\n", ftp_url, *msize);
...
int gf_ftp_download_to_file(const char *ftpurl, const char *local_name, const char *username, const char *password, const int timeout)
{
	...
	if(size > 0)
		GFFTPCLIENT_LOG_DEBUG( "download suc\turl[%s]\tfilesize:%d\n", ftp_url, size);
	else
		GFFTPCLIENT_LOG_DEBUG( "download err\turl[%s]\tfilesize:%d\n", ftp_url, size);
...
int gf_ftp_upload_from_file(const char *ftpurl, const char *local_name, const char *username, const char *password, const int timeout)
{
	...
	if(ret == 0)
		GFFTPCLIENT_LOG_DEBUG( "upload suc\turl[%s]\tlocalfile:%s\n", ftp_url, localname);
	else
		GFFTPCLIENT_LOG_DEBUG( "upload err\turl[%s]\tlocalfile:%s\n", ftp_url, localname);
...
int gf_ftp_upload_from_mem(const char *ftpurl, const char *membuf, unsigned long size, const char *username, const char *password, const int timeout)
{
	...
	if(ret == 0)
		GFFTPCLIENT_LOG_DEBUG( "upload suc\turl[%s]\n", ftp_url);
	else
		GFFTPCLIENT_LOG_DEBUG( "upload err\turl[%s]\n", ftp_url);
...
{  
	...
	if(argc < 2)
	{
		GFFTPCLIENT_LOG_DEBUG( "input error\n");
		return 0;
	}
	memset(localname, 0, sizeof(localname));
	if(argc == 2)
	...
	GFFTPCLIENT_LOG_DEBUG( "localname:%s\n", localname);
