...
static bool tcpclient_onrecv( uint64_t wparam, uint64_t lparam )
{
	...
	{
		GFTCPSERVER_LOG_DEBUG( "pclient == null\n");
		return false;
	}
	p = pClient->buf;
	...
	{	
		if(!pClient->pPacketHandler((HANDLE)pClient, pClient->ip, pClient->port, p, rcv_size, 0) )
		{
			...
			//pClient->lParam = 0;
			pClient->is_use = false;
			GFTCPSERVER_LOG_DEBUG( "packet handler return false\n");
			return false;
		}
	}
	if(rcv_size <= 0)
	{
		GFTCPSERVER_LOG_DEBUG( "recv size <= 0 %s:%d\n", __FUNCTION__, __LINE__);
		pClient->ip = 0;
		pClient->port = 0;
		if( pClient->socket != -1 )
		{
...
static bool tcpserver_proc(uint64_t lparam, uint64_t wparam)
{
	...
	for(i = 0; i < tcpserver->m_client_num; i ++) 
	{
		...
		{
			GFTCPSERVER_LOG_DEBUG( "this is a new client at position [%d]\n",i);
	...
	{
		GFTCPSERVER_LOG_DEBUG( "tcp client too many, more than %d\n",tcpserver->m_client_num);
		goto ERROR;
	}
	...
	{
		GFTCPSERVER_LOG_DEBUG( "TcpClientRecv open err\n");
		goto ERROR;
	}
	...
	GFTCPSERVER_LOG_DEBUG( "TcpClientRecv open suc\n");
...
STcpServer * gf_tcpserver_init(unsigned long ip, unsigned short port, PPacketHandler pPacketHandler,PErrorHandler pErrorHandler, unsigned long lParam)
{
    ...
    if(m_tcpserver == NULL)
    {
        GFTCPSERVER_LOG_DEBUG( "tcp server malloc error");
        return NULL;
    }
	...
	{
        GFTCPSERVER_LOG_DEBUG( "tcp server create socket error");
        return NULL;
	}
	if(gf_tcp_bind(m_tcpserver->socket, ip, port) < 0)
    {
        GFTCPSERVER_LOG_DEBUG( "tcp server bind socket error");
        return NULL;
    }

    if(gf_tcp_listen(m_tcpserver->socket,5) < 0)
    {
        GFTCPSERVER_LOG_DEBUG( "tcp server listen error");
        return NULL;
    }
