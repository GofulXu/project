...
int gf_udp_join( int skt, unsigned long ip )
{
	...
	if( IS_MULTICAST_IP(ip) )
	{
		...
		{
			GFUDP_LOG_DEBUG( "cannot add to multicast\n" );
			return -1;
		}
		if( setsockopt( skt, IPPROTO_IP, IP_MULTICAST_TTL, (char*)&ttl, sizeof(ttl) ) == SOCKET_ERROR )
		{
			GFUDP_LOG_DEBUG( "cannot set multicast args. \n" );
		}
		if( setsockopt( skt, IPPROTO_IP, IP_MULTICAST_LOOP, (char *)&flag, sizeof(flag) ) == SOCKET_ERROR )
		{
			GFUDP_LOG_DEBUG( "cannot set multicast ttl args. \n" );
		}
	}
	else
	{
		GFUDP_LOG_DEBUG( "not a multicast ip\n");
		return -1;
	}
...
int gf_udp_drop( int skt, unsigned long ip )
{
	...
	if( IS_MULTICAST_IP(ip) )
	{
		...
		{
			GFUDP_LOG_DEBUG( "cannot drop from multicast\n" );
			return -1;
		}
	}
