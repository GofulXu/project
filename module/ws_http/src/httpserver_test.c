...
{
	int recv = 0;
	int len = m_obj->request_header.content_length;
	FILE *fp = fopen("./mysave", "w");
	if(fp == NULL)return -1;
	//if(0 > sw_httpserver_send_response_header(m_obj, 200, "aaa", "bbb", "ccc", 1024, 3000))return -1;
	while(recv < len)
	{
		char buf[1024] = {0};
		int ret = sw_httpserver_recv_request_content(m_obj, buf, 1024, 3000);	
