...
//
//  File name: GPLE/alsa.c
//
//  Author: Vincent Lin (林世霖)  微信公众号：秘籍酷
//
//  Date: 2017-3
//  
//  Description: 利用ALSA库进行的音频处理模块
//
...
/*****************************************************************
 1)执行程序会提示多个*.conf文件缺失，按照错误提示将alsa/src/conf目录下
    的*.conf文件放置到开发板相应的目录
	
 2)执行程序错误时，再次提示缺失节点的问题，请执行以下操作:
mkdir /dev/snd 
...
int get_bits_per_sample(snd_pcm_format_t *snd_format)  
{     
    if (LE_SHORT(LE_SHORT(WAV_FMT_PCM)) != WAV_FMT_PCM)
        return -1;
      
    switch (LE_SHORT(LE_SHORT(16)))
    {  
    case 16:  
...
// 设置PCM格式参数
void set_wav_params(pcm_container *sound)
{
	...
	snd_pcm_hw_params_set_channels(sound->handle,
		hwparams, LE_SHORT(LE_SHORT(2)));
	sound->channels = LE_SHORT(LE_SHORT(2));

	// 6：设置采样频率
	// 注意：最终被设置的频率被存放在来exact_rate中
	uint32_t exact_rate = LE_INT(LE_INT(44100));
	...
	sound->bytes_per_frame =
		sound->bits_per_sample/8 * LE_SHORT(2);
...
ssize_t write_mpcm_to_device(pcm_container *pcm, size_t wcount)  
{  
    ssize_t r;  
    ssize_t result = 0;  
    uint8_t *data = pcm->period_buf;  
...
ssize_t read_mpcm_from_wav(int fd, void *buf, size_t count)  
{  
    ssize_t result = 0, res;  
...
int get_mbits_per_sample(wav_format *wav, snd_pcm_format_t *snd_format)  
{     
...
int set_mparams(pcm_container *pcm, wav_format *wav)  
{  
    snd_pcm_hw_params_t *hwparams;  
    ...
    get_mbits_per_sample(wav, &format);
...
int get_mwav_header_info(int fd, wav_format *wav)  
{  
...
void play_mmwav(pcm_container *pcm, wav_format *wav, int fd)  
{  
    int load, ret;  
    off64_t written = 0;  
    off64_t c;  
    ...
    while (written < total_bytes)
    {
        ...
        ret = write_mpcm_to_device(pcm, load);  
        if (ret != load)  
            break;  
...
/* // 从PCM设备录取音频数据，并发送
void wav_record(int fd, pcm_container *sound, wav_format *wav,uint32_t record_bytes)
{
	
	// 2：写PCM数据
	uint32_t nwrite = 0;

	while(total_bytes < record_bytes)
	{
...
/* 		uint32_t frames_read = read_pcm_data(sound, n);
		sendto(fd, sound->period_buf, frames_read * sound->bytes_per_frame);
...
/* 
void wave_end(int fd,wav_format *wav)
{
	lseek(fd, 0, SEEK_SET);
