...
{     
    ...
    pid_t pid;     
    int clientfd;
    ssize_t sendbytes,recvbytes;	 //定义客户端套接字    
    struct hostent *host;    
    char *buf,*buf_r;     
    if(argc < 4)    
    {      
    ...
    {       
        ...
        {           
            ...
            strcat(buf,":");
            memset(buf_r,0,100);    
            fgets(buf_r,100,stdin); //从标准输入获取
	    if(strstr(buf_r,"quit") != NULL)break;
            strncat(buf,buf_r,strlen(buf_r)-1);  //连接两个字符串         
        ...
        {    
            //子进程：接收信息        
            memset(buf,0,100);        
            if((recvbytes = recv(clientfd,buf,100,0)) <= 0)        
            {           
                perror("recv:");
                close(clientfd);          
                raise(SIGSTOP);          
