...
#define GF_FOURCC( a, b, c, d ) \
        ( ((uint32_t)d) | ( ((uint32_t)c) << 8 ) \
        | ( ((uint32_t)b) << 16 ) | ( ((uint32_t)a) << 24 ) )
#else
#define GF_FOURCC( a, b, c, d )   (((uint32_t)a) | ( ((uint32_t)b) << 8 ) | ( ((uint32_t)c) << 16 ) | ( ((uint32_t)d) << 24 ) )
#endif
...
#endif
	m_level = level;	
	gf_log_set_targets( targets);
	gf_log_set_mods( mods);
...
void gf_log_exit()
{
	...
	gf_log_clear_alltarget();
	gf_log_clear_allmods();
	while ( i++ < 5 && m_thrd)
	{
		m_b_exit_ftplog = true;
		if ( m_signal )
			gf_signal_give( m_signal );
		gf_thrd_delay(10);
	}
	if ( m_thrd )
		gf_thrd_close( m_thrd, 100 );
	if ( m_signal )
		gf_signal_destroy( m_signal );
	return ;
}
 ...
 *		for:ftp://ftpuser:111111@192.168.0.8/00-11-09-EF-B5-AF when update log gflog modules will pack as
...
int gf_log_add_target( char* target )
{
	...
	{/* 如果提前打开有可能打不开文件 */
		...
		{
			if ( m_thrd == NULL )
				m_thrd = gf_thrd_open("tFTPLogProc", 100, GF_SCHED_RR, 16*1024, (threadhandler_t)ftplog_proc, NULL, 0, 0);
			if ( m_signal == NULL )
				m_signal = gf_signal_create(0, 1);
		}
...
int gf_log_set_mods( char* mods )
{
	...
	while( *p != '\0' )
	{
		...
		if( m_mods_list[m_mods_num] == GF_FOURCC('a','l','l',0) )
...
int gf_log( int level,const char* mod, const char* file, int line, const char *format, ... )
{
	...
	//填充日志头信息
	size = snprintf(logbuf,LOG_BUF_SIZE,"[%u]%s[%s %s %d] ", gf_thrd_get_tick(), m_level_info[level],mod,file,line);
...
static uint32_t calc_imod(const char* mod)
{
	...
	return GF_FOURCC(t[0],t[1],t[2],t[3]);
}
/**
 *@brief 查找是否有相同的模块
 */
