 ...
 */
HANDLE  gf_signal_create( int defval, int maxval )
{
	gf_signal_t* h = (gf_signal_t*)gfos_malloc( sizeof( gf_signal_t ) );
 ...
 * @param signal 信号量句柄
 */
void gf_signal_destroy( HANDLE sem )
{
	if( sem == NULL )
	{
		return;
 ...
 */
int gf_signal_wait( HANDLE sem, int timeout )
{
	struct timespec tv;
 ...
 * @param signal 信号量句柄
 */
void gf_signal_give( HANDLE sem )
{
	int val;
	...
	if( val < ((gf_signal_t*)sem)->max_val )
	{
		sem_post( (sem_t*)sem );
	}
}
 ...
 * @param signal 信号量句柄
 */
void gf_signal_reset( HANDLE sem )
{
	gf_signal_give( sem );
}
