...
/*数据流接收线程*/
static bool udpclient_onrecv( uint64_t wparam, uint64_t lparam )
{
	...
	if(0 < gf_udp_select(pClient->socket, &readfds, NULL, NULL, 1000))
	{
		...
		if(rcv_size > 0)
		{
			...
			{	
				if(!pClient->pPacketHandler(pClient->socket, ip, port, p, pClient->datasize, pClient->lParam) )
				{
		...
		if(rcv_size <= 0)
		{
			...
			{
				if( 0 > pClient->pErrorHandler(pClient->socket, pClient->localip, pClient->localport, pClient->lParam) )
				{
					closesocket(pClient->socket);
					pClient->socket = INVALID_SOCKET;
					if(pClient->databuf)
...
int gf_udpclient_init( SUdpClient* pClient, unsigned long localip, unsigned short localport, unsigned long hostip, unsigned short hostport, PPacketHandler pPacketHandler, PErrorHandler pErrorHandler,unsigned long lParam )
{
	...
	memset( pClient, 0, sizeof(SUdpClient) );
	pClient->socket = INVALID_SOCKET;
	pClient->hThrd = NULL;
	pClient->databuf = NULL;
	pClient->mutex = NULL;
	pClient->localip = localip;
	pClient->localport = localport;
	pClient->hostip = hostip;
	pClient->hostport = hostport;
...
#endif
	if(0 > gf_udp_bind(skt, INADDR_ANY/*pClient->localip*/, pClient->localport))
	{
	...
	pClient->socket = skt;
	pClient->hThrd = gf_thrd_open( "udpClientRecv", 50, 0, 256*1024,/*(PThreadHandler)*/udpclient_onrecv, NULL, (uint64_t)pClient, lParam);
	if( pClient->hThrd == NULL )
	{
