...
int main(int argc, char *argv[])
{
	...
	if(argc < 2)
	{
		GFTFTPCLIENT_LOG_DEBUG( "pasge error\n");
		GFTFTPCLIENT_LOG_DEBUG( "please application -g + filename to download\t -p + filename to upload\n");
		return 0;
	}
	...
	if(success == 0)
	{
		GFTFTPCLIENT_LOG_DEBUG( "upload %s success\n", argv[2]);
		return 0;
	}
	GFTFTPCLIENT_LOG_DEBUG( "upload or download error ip:%s file:%s\n", argv[3], argv[2]);
	return 0;
}
#endif
