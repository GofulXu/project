...
HANDLE gf_thrd_open( char *name, unsigned char priority, gf_policy_t policy, int stack_size, threadhandler_t handler, threadhandler_t Errorhandler, uint64_t wparam, uint64_t lparam )
{
...
#endif
			{
				GFTHRD_LOG_DEBUG( "Release self-exit thread<%s:%x>\n", m_all[i]->name, *(uint32_t *)&m_all[i] );			
	...
	{
		GFTHRD_LOG_DEBUG( "Thread num reach max(%d)!!\n", MAX_THRD_NUM );
		gf_mutex_unlock( m_mutex );

		return NULL;
	}
	info = (sthrdinfo_t*)gfos_malloc( sizeof(sthrdinfo_t) );
	
	if( info == NULL )
	{
		GFTHRD_LOG_DEBUG( "error: gf_thrd_open() failed\n" );
		if( m_mutex )
		{
			gf_mutex_unlock( m_mutex );
		}
	...
	if( info->policy < GF_SCHED_NORMAL || info->policy > GF_SCHED_RR )
	{
		info->policy = GF_SCHED_NORMAL;
	}
	/* 初始化线程属性对象 */
	pattr = &( info->attr );
	...
	if( rc != 0 )
	{
		...
		GFTHRD_LOG_DEBUG( "gf_thrd_open [%s]: set stack size failed! %d, using default size %ld\n", name, stack_size, defsize );
	}
	...
	GFTHRD_LOG_DEBUG( "Thread-<%s : %d : %x> opened\n", info->name,i,*(uint32_t *)info );
 ...
 */
void gf_thrd_close( HANDLE thrd, int ms )
{
	int i = 0;
	int status = 0;
	void *result;
	...
	if( !thrd )
	{
		GFTHRD_LOG_DEBUG( "error!thrd is NULL!\n" );
		return;
	}
	...
	GFTHRD_LOG_DEBUG( "Thread-<%s : %d : %x> closing [%d]...\n", info->name,i,*(uint32_t*)info, ms );
	...
	if( info->handler )
	{
		...
		{
			GFTHRD_LOG_DEBUG( "Thread<%s:%x> is kill!!!\n", info->name, *(uint32_t *)info );
			usleep( 10*1000 );
		}
#else
		/* 强制退出 */
		...
		{
			GFTHRD_LOG_DEBUG( "Thread<%s:%x> is canneled!!!\n", info->name, *(uint32_t *)info );
		}
		else
		{
			...
			if( status == 0 )
			{
				...
				{
					GFTHRD_LOG_DEBUG( "Thread<%s:%x> canceled!!!\n", info->name, *(uint32_t *)info);//如果线程是被取消的，回打印。
				}
				else
				{
					GFTHRD_LOG_DEBUG( "result:%d\n", *(int *)result);
				}
			}
		}
#endif
 ...
 */
unsigned int gf_thrd_is_openned( HANDLE thrd )
{
	if( !thrd )
	{
		GFTHRD_LOG_DEBUG( "error!thrd is NULL!" );
 ...
 */
bool gf_thrd_is_running( HANDLE thrd )
{
	if( !thrd )
	{
		GFTHRD_LOG_DEBUG( "error!thrd is NULL!" );
 ...
 */
void gf_thrd_set_priority( HANDLE thrd, unsigned char priority )
{
	if( !thrd )
	{
		GFTHRD_LOG_DEBUG( "error!thrd is NULL!" );
 ...
 * @return 当前线程的优先级
 */
unsigned char gf_thrd_get_priority( HANDLE thrd )
{
	if( !thrd )
	{
		GFTHRD_LOG_DEBUG( "error!thrd is NULL!" );
 ...
 * @param thrd 线程句柄 
 */
void gf_thrd_pause( HANDLE thrd )
{
	if( !thrd )
	{
		GFTHRD_LOG_DEBUG( "error!thrd is NULL!" );
 ...
 */
bool gf_thrd_is_paused( HANDLE thrd )
{
	if( !thrd )
	{
		GFTHRD_LOG_DEBUG( "error!thrd is NULL!" );
 ...
 * @param thrd 线程句柄 
 */
void gf_thrd_resume( HANDLE thrd )
{
	if( !thrd )
	{
		GFTHRD_LOG_DEBUG( "error!thrd is NULL!" );
 ...
 */
void gf_thrd_delay( int timeout )
{
	usleep( timeout*1000 );
 ...
 * @return 线程句柄
 */
static HANDLE gf_thrd_find_pidname(pid_t pid)
{
	int i;
...
{   
	HANDLE thrd = gf_thrd_find_pidname(getpid());
	sthrdinfo_t *info = (sthrdinfo_t *)thrd;
	GFTHRD_LOG_DEBUG( "On Live:Thread<%s:%x> is exit!!!\n", info->name, *(uint32_t *)info );
	pthread_exit(0);  
}  
#endif
/** 
...
static void* sema_thread_handler( void* lp_param )
{
	...
	{
		GFTHRD_LOG_DEBUG("sigaction failed !\n");
	}  
#endif
	...
	if(NULL != info->Errorhandler)
		info->Errorhandler(info->wparam, info->lparam);
	info->handler = NULL;
	GFTHRD_LOG_DEBUG( "Thread-<%s:%x> exit self\n", info->name, *(uint32_t *)info );
 ...
 * @brief 设置全局调度策略
 * 
 * @param policy gf_policy_t
 */	
void gf_thrd_set_global_policy( gf_policy_t policy )
{
...
void gf_thrd_print()
{
	...
	for( i=0; i<MAX_THRD_NUM; i++ )
	{
		...
		{
			GFTHRD_LOG_DEBUG( "%-2d 0x%08x %-14s ppid=%d pid=%d proc=0x%-8x policy=%d priority=%d pause=%d \n", 
				   			i, *(uint32_t *)(m_all[i]), m_all[i]->name,m_all[i]->ppid, m_all[i]->pid, *(uint32_t *)m_all[i]->handler, 
 ...
 * @return 返回填充buf的大小
 */
int gf_thrd_getinfo( char* buf, int size )
{
	char sz_buf[255];
	int i, n, len;
	...
	*buf = 0;
	...
	for( i=0; i<MAX_THRD_NUM; i++ )
	{
		if( m_all[i] && m_all[i]->handler )
		{
			n = sprintf( sz_buf, "%-2d 0x%08x %-14s ppid=%d pid=%d proc=0x%-8x policy=%d priority=%-3d pause=%d \n", 
				   i, *(uint32_t *)(m_all[i]), m_all[i]->name,m_all[i]->ppid, m_all[i]->pid,*(uint32_t *)m_all[i]->handler,
				   m_all[i]->policy, m_all[i]->priority, m_all[i]->is_pause);
			if( len + n < size )
			{
				strcpy( buf+len, sz_buf );
				len += n;
			}
		}
	}
 ...
 * @return 线程句柄
 */
HANDLE gf_thrd_find_byname( char* name )
{
	int i;
