...
/* 打开线程，打开后处于暂停状态 */
HANDLE gf_thrd_open( char *name, unsigned char priority, int policy, int stack_size, 
					PThreadHandler pHandler, unsigned long wParam, unsigned long lParam )
{
	...
	info = gfos_malloc(sizeof(SThrdInfo));
	...
	if(info->m_policy < GF_SCHED_NORMAL || info->m_policy > GF_SCHED_RR)
		info->m_policy = GF_SCHED_NORMAL;
	...
	if(rc != 0)
	{
		...
		pthread_attr_getstacksize(pAttr, (size_t *)&defsize);
		printf("gf_thrd_open [%s]: set stack size failed! %d, using default size %d\n", name, stack_size, defsize);
	}
...
/* 关闭线程 */
void gf_thrd_close( HANDLE hThrd, int ms )
{
	int i = 0;
...
/* 是否打开 */
unsigned int gf_thrd_is_openned( HANDLE hThrd )
{
	SThrdInfo *info = (SThrdInfo *)hThrd;
	if( info == NULL )
		return false;
...
/* 设置线程优先级  */
void gf_thrd_set_priority( HANDLE hThrd, unsigned char priority )
{
	struct sched_param param;
	SThrdInfo *info = (SThrdInfo *)hThrd;
	if( info == NULL )
 ...
 * @return 当前线程的优先级
 */
unsigned char gf_thrd_get_priority( HANDLE hThrd )
{
	SThrdInfo *info = (SThrdInfo *)hThrd;
...
/* 暂停 */
void gf_thrd_pause( HANDLE hThrd )
{
	SThrdInfo *info = (SThrdInfo *)hThrd;
	if( info )
		info->bPause = 1;
...
bool gf_thrd_is_paused(HANDLE hThrd)
{
...
/* 继续 */
void gf_thrd_resume( HANDLE hThrd )
{
	SThrdInfo *info = (SThrdInfo *)hThrd;
	if( info )
	{
...
void gf_thrd_set_global_policy(int policy)
{
